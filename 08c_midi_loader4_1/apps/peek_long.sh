...
main() {
	...
	sh2_shift_right_logical_16 r2

	# 2バイト目(b23-16)をr3へ設定
	sh2_copy_to_reg_from_reg r3 r1
	sh2_shift_right_logical_16 r3

	# 3バイト目(b15-8)をr4へ設定
	sh2_copy_to_reg_from_reg r4 r1
	sh2_shift_right_logical_8 r4
	...
	## 3バイト目
	sh2_and_to_reg_from_reg r4 r0
	## 4バイト目
	sh2_and_to_reg_from_reg r5 r0
	...
	cat apps/peek_long.1.o
	local sz_1=$(stat -c '%s' apps/peek_long.1.o)
	#### T == 1なら繰り返す
	...
	sh2_rel_jump_if_true $(two_comp_d $(((4 + sz_1) / 2)))
	### MOBUFへ書き込み
	sh2_copy_to_ptr_from_reg_byte r13 r4
	## データバイト(4バイト目)送信
	### MCIPDのMOビットが設定されるのを待つ
	cat apps/peek_long.1.o
	sh2_rel_jump_if_true $(two_comp_d $(((4 + sz_1) / 2)))
	### MOBUFへ書き込み
	sh2_copy_to_ptr_from_reg_byte r13 r5
	## パディング送信
	### MCIPDのMOビットが設定されるのを待つ
	cat apps/peek_long.1.o
	sh2_rel_jump_if_true $(two_comp_d $(((4 + sz_1) / 2)))
