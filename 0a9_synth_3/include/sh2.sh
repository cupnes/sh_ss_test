...
sh2_copy_to_reg_from_ptr_and_inc_ptr_word() {
	local dst=$1
	local src=$2
	local dstnum=$(to_regnum $dst)
	local srcnum=$(to_regnum $src)
	echo -en "\x6${dstnum}\x${srcnum}5"	# mov.w @$src+,$dst
	echo -e "mov.w @$src+,$dst\t;1" >>$ASM_LIST_FILE
}
...
# r0 == val のとき 1→T
sh2_compare_r0_eq_val() {
	local val=$1
	echo -en "\x88\x${val}"	# cmp/eq val,r0
	echo -e "cmp/eq val,r0\t;1" >>$ASM_LIST_FILE
}
...
# dst = ~src
sh2_not_to_reg_from_reg() {
	local dst=$1
	local src=$2
	local dstnum=$(to_regnum $dst)
	local srcnum=$(to_regnum $src)
	echo -en "\x6${dstnum}\x${srcnum}7"	# not $src,$dst
	echo -e "not $src,$dst\t;1" >>$ASM_LIST_FILE
}
...
sh2_shift_right_arithmetic() {
	local reg=$1
	local regnum=$(to_regnum $reg)
	echo -en "\x4${regnum}\x21"	# shar $reg
	echo -e "shar $reg\t;1" >>$ASM_LIST_FILE
}
...
sh2_shift_right_logical_2() {
	local reg=$1
	local regnum=$(to_regnum $reg)
	echo -en "\x4${regnum}\x09"	# shlr2 $reg
	echo -e "shlr2 $reg\t;1" >>$ASM_LIST_FILE
}
