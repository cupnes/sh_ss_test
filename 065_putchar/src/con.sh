...
f_backspace_cursor() {
	...
	# con領域に2文字以上存在するか?
	## VRAM_CT_CON_BASE(r2) += SS_VDP1_COMMAND_SIZE * 2
	sh2_add_to_reg_from_val_byte r2 $(calc16_2 "${SS_VDP1_COMMAND_SIZE}*2")
	## var_next_vdpcom_con_addr(r1) >= r2 ?
	sh2_compare_reg_ge_reg_unsigned r1 r2
	(
		# var_next_vdpcom_con_addr(r1) < r2 の場合
	...
	# var_next_vdpcom_con_addr(r1) >= r2 (T==1)の場合
	cat src/f_backspace_cursor.4.o
	# var_next_vdpcom_con_addr(r1) < r2 (T==0)の場合
